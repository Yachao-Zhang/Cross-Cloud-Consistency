...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            if self.config.saving:
                ...
                self.Log_file.write(' '.join(
                    ["config.%s = %s\n" % (k, v) for k, v in self.config.__dict__.items() if
                     not k.startswith('__')]))
        ...
        with tf.variable_scope('loss'):
            ...
            valid_labels, valid_logits, _ = self.data_prep(self.logits, self.embedding, self.labels, d)
            self.label_loss = self.get_loss(valid_logits, valid_labels, self.class_weights)
